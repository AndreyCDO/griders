...
TOOLS: list[Tool] = [
    ...
    Tool(
        name="ghost_open_deal",
        description="Build and optionally send a Cryptorg Ghost Bot open webhook. Dry-run unless confirm=true.",
        inputSchema=obj({
            "pair": {"type": "string"},
            "strategy": {"type": "string", "enum": ["long", "short"]},
            "order_volume": {"type": "string"},
            "leverage": {"type": "integer"},
            "margin_type": {"type": "string", "default": "cross"},
            "order_type": {"type": "string", "default": "Market"},
            "cycles": {"type": "integer", "default": 1},
            "dca_enabled": {"type": "boolean"},
            "dca_max": {"type": "integer"},
            "dca_active": {"type": "integer"},
            "dca_volume": {"type": "string"},
            "dca_percent": {"type": "string"},
            "dca_multiplier_volume": {"type": "string"},
            "dca_multiplier_price": {"type": "string"},
            "close_enabled": {"type": "boolean", "default": True},
            "close_value": {"type": "string"},
            "stop_enabled": {"type": "boolean", "default": True},
            "stop_value": {"type": "string"},
            "stop_delay": {"type": "integer", "default": 3},
            "confirm": {"type": "boolean", "default": False},
        }, ["pair", "strategy"]),
    ),
    Tool(
        name="ghost_send_payload",
        description="Send an arbitrary JSON payload to the configured Cryptorg Ghost Bot webhook. Dry-run unless confirm=true.",
        inputSchema=obj({
            "payload": {"type": "object"},
            "confirm": {"type": "boolean", "default": False},
        }, ["payload"]),
    ...
    Tool(
        name="ghost_modify_deal",
        description="Build and optionally send a Cryptorg Ghost Bot update webhook for DCA, take-profit, or stop-loss. Dry-run unless confirm=true.",
        inputSchema=obj({
            "pair": {"type": "string"},
            "strategy": {"type": "string", "enum": ["long", "short"]},
            "bot_id": {"type": ["integer", "string"]},
            "dca_enabled": {"type": "boolean"},
            "dca_max": {"type": "integer"},
            "dca_active": {"type": "integer"},
            "dca_volume": {"type": "string"},
            ...
            "dca_multiplier_volume": {"type": "string"},
            "dca_multiplier_price": {"type": "string"},
            "close_enabled": {"type": "boolean"},
            "close_value": {"type": "string"},
            "stop_enabled": {"type": "boolean"},
            "stop_value": {"type": "string"},
            "stop_delay": {"type": "integer"},
            "confirm": {"type": "boolean", "default": False},
        }, ["pair", "strategy"]),
    ),
    Tool(
        name="ghost_average_deal",
        description="Build and optionally send a Cryptorg Ghost Bot averaging webhook. Dry-run unless confirm=true.",
        inputSchema=obj({
            "pair": {"type": "string"},
            "strategy": {"type": "string", "enum": ["long", "short"]},
            "amount": {"type": "string"},
            "bot_id": {"type": ["integer", "string"]},
            "confirm": {"type": "boolean", "default": False},
        }, ["pair", "strategy", "amount"]),
    ),
    Tool(
        name="ghost_close_deal",
        description="Build and optionally send a Cryptorg Ghost Bot close webhook. Dry-run unless confirm=true.",
        inputSchema=obj({
            "pair": {"type": "string"},
            "strategy": {"type": "string", "enum": ["long", "short"]},
            "close_position": {"type": "boolean", "default": True},
            "bot_id": {"type": ["integer", "string"]},
            "confirm": {"type": "boolean", "default": False},
        }, ["pair", "strategy"]),
    ),
...
async def _dispatch(name: str, args: dict) -> Any:
    ...
    if name == "ghost_open_deal":
        return await ghost_webhook.open_deal(
            pair=args["pair"],
            strategy=args["strategy"],
            order_volume=args.get("order_volume"),
            leverage=args.get("leverage"),
            margin_type=args.get("margin_type"),
            order_type=args.get("order_type"),
            cycles=args.get("cycles"),
            dca_enabled=args.get("dca_enabled"),
            dca_max=args.get("dca_max"),
            dca_active=args.get("dca_active"),
            dca_volume=args.get("dca_volume"),
            dca_percent=args.get("dca_percent"),
            dca_multiplier_volume=args.get("dca_multiplier_volume"),
            dca_multiplier_price=args.get("dca_multiplier_price"),
            close_enabled=args.get("close_enabled", True),
            close_value=args.get("close_value"),
            stop_enabled=args.get("stop_enabled"),
            stop_value=args.get("stop_value"),
            stop_delay=args.get("stop_delay"),
            confirm=args.get("confirm", False),
        )
    if name == "ghost_send_payload":
        return await ghost_webhook.send_payload(
            payload=args["payload"],
            confirm=args.get("confirm", False),
        )
    if name == "ghost_modify_deal":
        return await ghost_webhook.modify_deal(
            pair=args["pair"],
            strategy=args["strategy"],
            ...
            dca_multiplier_volume=args.get("dca_multiplier_volume"),
            dca_multiplier_price=args.get("dca_multiplier_price"),
            close_enabled=args.get("close_enabled"),
            close_value=args.get("close_value"),
            stop_enabled=args.get("stop_enabled"),
            stop_value=args.get("stop_value"),
            stop_delay=args.get("stop_delay"),
    ...
    if name == "ghost_average_deal":
        return await ghost_webhook.average_deal(
            pair=args["pair"],
            strategy=args["strategy"],
            amount=args["amount"],
            bot_id=args.get("bot_id"),
    ...
    if name == "ghost_close_deal":
        return await ghost_webhook.close_deal(
            pair=args["pair"],
            strategy=args["strategy"],
            close_position=args.get("close_position", True),
            bot_id=args.get("bot_id"),
            confirm=args.get("confirm", False),
        )
