...
`griders_grid_dca_v28.pine` sends only `grid_dca_v2` signals for the current public GRID DCA 2.8 strategy.

Replace the old indicator on each chart with the GRID DCA 2.8 script and keep the alert condition as `Any alert() function call`.
...
2. Add `griders_grid_dca_v28.pine` as an indicator.
