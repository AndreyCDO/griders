...
SESSION_NAME="griders_webinar_bot"
MODE="${1:-restart}"
LOG_FILE="$APP_DIR/webinar_bot.log"
...
if [ "$MODE" = "install-autostart" ]; then
  marker="griders_webinar_bot_autostart"
  job="@reboot cd $APP_DIR && ./deploy_webinar_bot.sh restart # $marker"
...
screen -dmS "$SESSION_NAME" bash -lc \
  "cd '$APP_DIR' && ./venv/bin/python webapp/webinar_bot.py >> '$LOG_FILE' 2>&1"
